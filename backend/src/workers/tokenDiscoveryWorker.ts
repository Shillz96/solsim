  ...
  try {
    ...
    for (const token of tokens) {
      try {
        ...
        // CRITICAL FIX (2025-10-31): Use raw SQL to bypass @updatedAt trigger entirely
        // updateMany() still updates lastUpdatedAt in Prisma 5.x, causing checkpoint storms
        // Raw SQL gives us complete control over which fields are modified
    ...
    for (const token of activeTokens) {
      try {
        ...
        if (holderCount !== null && holderCount !== undefined) {
          // CRITICAL FIX (2025-10-31): Use raw SQL to avoid triggering @updatedAt
          // Holder counts update every 10min for 100 tokens = 600/hour writes
          // This was a major contributor to checkpoint storms
          await prisma.$executeRaw`
            UPDATE "TokenDiscovery"
            SET "holderCount" = ${holderCount}
            WHERE "mint" = ${token.mint}
          `;
          updated++;
...
async function syncWatcherCounts(): Promise<void> {
  try {
    ...
    for (const { mint, _count } of counts) {
      // CRITICAL FIX (2025-10-31): Use raw SQL to avoid triggering @updatedAt
      // Watcher counts update every 5min, contributing to checkpoint storms
      await prisma.$executeRaw`
        UPDATE "TokenDiscovery"
        SET "watcherCount" = ${_count.mint}
        WHERE "mint" = ${mint}
      `;
      updated++;
    }
