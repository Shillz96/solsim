"use client"

import { Card } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { TrendingUp, TrendingDown, Activity } from "lucide-react"

export type PortfolioFilterType = 'all' | 'gainers' | 'losers';

interface PortfolioFiltersProps {
  onFilterChange?: (filter: PortfolioFilterType) => void;
  activeFilter?: PortfolioFilterType;
}

export function PortfolioFilters({ 
  onFilterChange, 
  activeFilter = 'all' 
}: PortfolioFiltersProps) {
  const handleFilterClick = (filter: PortfolioFilterType) => {
    if (onFilterChange) {
      onFilterChange(filter);
    }
  };

  return (
    <Card className="p-4">
      <h3 className="text-sm font-semibold mb-3">Quick Filters</h3>
      <div className="space-y-2">
        <Button 
          variant={activeFilter === 'all' ? 'default' : 'ghost'} 
          className="w-full justify-start text-sm" 
          size="sm"
          onClick={() => handleFilterClick('all')}
        >
          <Activity className="h-4 w-4 mr-2" />
          All Positions
        </Button>
        <Button 
          variant={activeFilter === 'gainers' ? 'default' : 'ghost'}
          className="w-full justify-start text-sm" 
          size="sm"
          onClick={() => handleFilterClick('gainers')}
        >
          <TrendingUp className="h-4 w-4 mr-2 text-secondary" />
          Gainers
        </Button>
        <Button 
          variant={activeFilter === 'losers' ? 'default' : 'ghost'}
          className="w-full justify-start text-sm" 
          size="sm"
          onClick={() => handleFilterClick('losers')}
        >
          <TrendingDown className="h-4 w-4 mr-2 text-destructive" />
          Losers
        </Button>
      </div>
    </Card>
  )
}

