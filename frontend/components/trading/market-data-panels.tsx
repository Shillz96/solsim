...
const RecentTradesPanel = memo(function RecentTradesPanel({ tokenMint }: { tokenMint: string }) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 h-full overflow-hidden flex flex-col shadow-[3px_3px_0_var(--outline-black)]">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-8 h-8 rounded-lg bg-luigi border-3 border-outline flex items-center justify-center shadow-[2px_2px_0_var(--outline-black)]">
          <TrendingUp className="w-4 h-4 text-white" />
        </div>
        <div>
          <h3 className="font-mario font-bold text-xs uppercase text-outline">Recent Market Activity</h3>
          <p className="text-[10px] text-foreground/70 font-bold">
      ...
      {isLoading ? (
        ...
        <div className="space-y-1.5 overflow-y-auto flex-1">
          {trades.map((trade, index) => {
            ...
            return (
              <div key={index} className="bg-white border-3 border-outline rounded-lg p-2 flex items-center gap-2 shadow-[2px_2px_0_var(--outline-black)]">
                <div className={cn(
                  "w-7 h-7 rounded-lg border-3 border-outline flex items-center justify-center shadow-[2px_2px_0_var(--outline-black)]",
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="font-mario font-bold text-xs text-outline">{isBuy ? 'BUY' : 'SELL'}</span>
                    <span className="text-xs font-mono font-bold">{formatNumber(trade.amountSol || 0)} SOL</span>
                  </div>
                  <div className="text-[10px] text-foreground/70 truncate font-bold">
...
const TopTradersPanel = memo(function TopTradersPanel({ tokenMint }: { tokenMint: string }) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 shadow-[3px_3px_0_var(--outline-black)]">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-8 h-8 rounded-lg bg-star border-3 border-outline flex items-center justify-center shadow-[2px_2px_0_var(--outline-black)]">
          <Users className="w-4 h-4 text-outline" />
        </div>
        <div className="flex-1">
          <h3 className="font-bold text-xs uppercase">Top Traders</h3>
          <div className="flex items-center gap-2">
            <p className="text-[10px] text-foreground opacity-70">Helius on-chain data</p>
            {!isLoading && traders.length > 0 && (
              <span className="inline-flex items-center gap-1 text-[9px] text-luigi font-bold">
          ...
          {traders.map((trader: any, index: number) => (
            ...
            >
              ...
              <div className="text-right">
                <div className={`text-xs font-bold ${trader.profitSol >= 0 ? 'text-luigi' : 'text-mario'}`}>
                  {trader.profitSol >= 0 ? '+' : ''}{trader.profitSol.toFixed(3)} SOL
                </div>
                <div className="text-[10px] opacity-70">{trader.trades} trades</div>
              </div>
            </div>
          ))}
        </div>
...
const HoldersPanel = memo(function HoldersPanel({ tokenMint }: { tokenMint: string }) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 shadow-[3px_3px_0_var(--outline-black)]">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-8 h-8 rounded-lg bg-coin border-3 border-outline flex items-center justify-center shadow-[2px_2px_0_var(--outline-black)]">
          <Wallet className="w-4 h-4 text-outline" />
        </div>
        <div>
          <h3 className="font-mario font-bold text-xs uppercase text-outline">Top Holders</h3>
          {holderData && (
            <p className="text-[10px] text-foreground/70 font-bold">
      ...
      {isLoading ? (
        ...
        <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
          {holderData.holders.map((holder) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="font-bold text-xs">#{holder.rank}</span>
                <span className="text-[10px] font-mono">
...
const BubbleMapsPanel = memo(function BubbleMapsPanel({ tokenMint }: { tokenMint: string }) {
  ...
  if (isAvailable === null) {
    return (
      <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 shadow-[3px_3px_0_var(--outline-black)]">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-8 h-8 rounded-lg bg-super border-3 border-outline flex items-center justify-center shadow-[2px_2px_0_var(--outline-black)]">
            <Network className="w-4 h-4 text-white" />
          </div>
          <div>
            <h3 className="font-mario font-bold text-xs uppercase text-outline">Bubble Map</h3>
            <p className="text-[10px] text-foreground/70 font-bold">Checking...</p>
          </div>
        </div>
        <div className="text-center py-4">
          <div className="text-2xl mb-2">⏳</div>
          <p className="text-xs text-foreground/70 font-bold">Loading...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 shadow-[3px_3px_0_var(--outline-black)]">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h3 className="font-mario font-bold text-xs uppercase text-outline">Bubble Map</h3>
            <p className="text-[10px] text-foreground/70 font-bold">Not available</p>
          </div>
        </div>
        <div className="text-center py-4">
          <div className="text-2xl mb-2">🫧</div>
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border-3 border-outline rounded-lg p-3 shadow-[3px_3px_0_var(--outline-black)]">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          <h3 className="font-mario font-bold text-xs uppercase text-outline">Bubble Map</h3>
          <p className="text-[10px] text-foreground/70 font-bold">Interactive visualization</p>
        </div>
      </div>
