  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        color: isPositive ? "text-luigi" : "text-mario",
        bgColor: isPositive ? "bg-luigi/10" : "bg-mario/10",
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="mario-card bg-card border-3 border-outline shadow-[3px_3px_0_var(--outline-black)] p-4">
            <div className="flex items-start justify-between">
              ...
              <div className="p-2 rounded-lg border-2 border-outline bg-[var(--pipe-100)]">
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {stats.map((stat, index) => (
        <div
          key={index}
          className="bg-sky/20 border-4 border-outline rounded-xl shadow-[4px_4px_0_var(--outline-black)] p-4 transition-transform hover:scale-105 hover:shadow-[6px_6px_0_var(--outline-black)] cursor-default"
        >
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <p className="text-xs font-bold text-outline">{stat.label}</p>
              <p className="text-xl font-bold text-outline">{stat.value}</p>
            </div>
            <div className="bg-sky border-2 border-outline rounded-lg p-2">
              <stat.icon className="h-5 w-5 text-outline" />
